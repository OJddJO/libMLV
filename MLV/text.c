...
void init_font() {
    if (TTF_Init() < 0) {
        fprintf(stderr, "Couldn't initialize TTF: %s - file : %s, ligne : %i\n", SDL_GetError(), __FILE__, __LINE__);
        exit(1);
    }
}
...
MLVAPI MLV_Font *MLV_load_font(const char *file_font, int size) {
    ...
    if (!font->font) {
        printf("TTF_OpenFont: %s - file : %s, ligne : %i\n", TTF_GetError(), __FILE__, __LINE__);
        exit(1);
    }
...
void drawString(SDL_Surface *textBox, int x, int y, const char *message, MLV_Color color, TTF_Font *font) {
    ...
    SDL_Surface *text;
    if (strlen(message)) {
        text = TTF_RenderUTF8_Solid(font, message, sdl_color);
    } else {
        text = TTF_RenderUTF8_Solid(font, " ", sdl_color);
    }
...
int internal_draw_adapted_text_box_size_with_font(char *copy, const MLV_Font *font, int sizeInterligne, int *width,
    int *height, int *width_text) {
    int nb_line = 1;
    *width = 0;
    *height = 0;
    if (copy[0] != '\0') {
        ...
        while ((new = strchr(new, '\n'))) {
            ...
            if (width_line > *width) { *width = width_line; }
    ...
    if (width_text) { *width_text = *width; }
    // Ajout des bords
    ...
    for (i = 0; i < nb_line; i++) {
        ...
        switch (text_justification) {
            case MLV_TEXT_CENTER:
                drawString(textBox, horizontal_offset + (width_text - width_line) / 2, position, old, textColor,
                    font->font);
                break;
            case MLV_TEXT_LEFT:
                drawString(textBox, horizontal_offset, position, old, textColor, font->font);
                break;
            case MLV_TEXT_RIGHT:
                drawString(textBox, horizontal_offset + width_text - width_line + sizeInterligne, position, old,
                    textColor, font->font);
                break;
            default:
...
void internal_draw_adapted_text_box_with_font_on_image(int x, int y, const char *message, const MLV_Font *font,
    int sizeInterligne, MLV_Color borderColor, MLV_Color textColor, MLV_Color backgroundColor,
    MLV_Text_justification text_justification, SDL_Surface *screen) {
    char *copy = (char *)MLV_MALLOC(strlen(message) + 1, char);
    snprintf(copy, strlen(message) + 1, "%s", message);
    int width, height, width_text;

    int nb_line =
        internal_draw_adapted_text_box_size_with_font(copy, font, sizeInterligne, &width, &height, &width_text);
    ...
    SDL_Surface *textBox = create_surface(width, height);
...
MLVAPI void MLV_draw_adapted_text_box_with_font_va(int x, int y, const char *message, const MLV_Font *font, int sizeInterligne,
    ...
    internal_draw_adapted_text_box_with_font_on_image(x, y, complete_message, font, sizeInterligne, borderColor,
        textColor, backgroundColor, text_justification, MLV_data->screen);
...
MLVAPI void MLV_draw_adapted_text_box(int x, int y, const char *message, int sizeInterligne, MLV_Color borderColor,
    ...
    MLV_draw_adapted_text_box_va(x, y, message, sizeInterligne, borderColor, textColor, backgroundColor,
...
MLVAPI void MLV_draw_adapted_text_box_with_font_on_image_va(int x, int y, const char *message, const MLV_Font *font,
    ...
    if (vasprintf(&complete_message, message, pile) == -1) { ERROR("Unexpected Error."); }
    internal_draw_adapted_text_box_with_font_on_image(x, y, complete_message, font, sizeInterligne, borderColor,
        textColor, backgroundColor, text_justification, image->surface);
    free(complete_message);
}
...
MLVAPI void MLV_draw_adapted_text_box_on_image(int x, int y, const char *message, int sizeInterligne, MLV_Color borderColor,
    ...
    va_list pile;
    va_start(pile, image);
    MLV_draw_adapted_text_box_on_image_va(x, y, message, sizeInterligne, borderColor, textColor, backgroundColor,
        text_justification, image, pile);
    va_end(pile);
}
...
MLVAPI int MLV_get_size_of_adapted_text_box_with_font_va(const char *message, const MLV_Font *font, int sizeInterligne,
    ...
    int result =
        internal_draw_adapted_text_box_size_with_font(copy, font, sizeInterligne, result_width, result_height, NULL);
    MLV_FREE(copy, char);
    free(complete_message);
    return result;
}
...
MLVAPI int MLV_get_size_of_adapted_text_box(const char *message, int sizeInterligne, int *result_width, int *result_height,
    ...
    result = MLV_get_size_of_adapted_text_box_va(message, sizeInterligne, result_width, result_height, pile);
    va_end(pile);
    return result;
}
...
MLVAPI void MLV_draw_adapted_text_box_with_font(int x, int y, const char *message, const MLV_Font *font, int sizeInterligne,
    ...
    va_list pile;
    va_start(pile, text_justification);
    MLV_draw_adapted_text_box_with_font_va(x, y, message, font, sizeInterligne, borderColor, textColor, backgroundColor,
        text_justification, pile);
    va_end(pile);
}
...
MLVAPI void MLV_draw_adapted_text_box_with_font_on_image(int x, int y, const char *message, const MLV_Font *font,
    ...
    va_list pile;
    va_start(pile, image);
    MLV_draw_adapted_text_box_with_font_on_image_va(x, y, message, font, sizeInterligne, borderColor, textColor,
...
MLVAPI int MLV_get_size_of_adapted_text_box_with_font(const char *message, const MLV_Font *font, int sizeInterligne,
    ...
    va_start(pile, result_height);
    result =
        MLV_get_size_of_adapted_text_box_with_font_va(message, font, sizeInterligne, result_width, result_height, pile);
    va_end(pile);
    return result;
}

void internal_draw_text_box_with_font_on_image(int x, int y, int width, int height, const char *message,
    const MLV_Font *font, int sizeInterligne, MLV_Color borderColor, MLV_Color textColor, MLV_Color backgroundColor,
    MLV_Text_justification text_justification, MLV_Horizontal_position horizontal_position,
    MLV_Vertical_position vertical_position, SDL_Surface *image) {
    SDL_Rect rectangle;
    rectangle.x = x;
    rectangle.y = y;
    rectangle.w = width;
    ...
    int nb_line = internal_draw_adapted_text_box_size_with_font(copy, font, sizeInterligne, &real_width, &real_height,
        &width_text);
...
MLVAPI void MLV_draw_text_box_with_font_va(int x, int y, int width, int height, const char *message, const MLV_Font *font,
    ...
    char *complete_message;
    if (vasprintf(&complete_message, message, pile) == -1) { ERROR("Unexpected Error."); }
    internal_draw_text_box_with_font_on_image(x, y, width, height, complete_message, font, sizeInterligne, borderColor,
        textColor, backgroundColor, text_justification, horizontal_position, vertical_position, MLV_data->screen);
    free(complete_message);
}
...
MLVAPI void MLV_draw_text_box(int x, int y, int width, int height, const char *message, int sizeInterligne,
    ...
    MLV_draw_text_box_va(x, y, width, height, message, sizeInterligne, borderColor, textColor, backgroundColor,
        text_justification, horizontal_position, vertical_position, pile);
    va_end(pile);
}
...
MLVAPI void MLV_draw_text_box_with_font(int x, int y, int width, int height, const char *message, const MLV_Font *font,
    ...
    va_list pile;
    va_start(pile, vertical_position);
    MLV_draw_text_box_with_font_va(x, y, width, height, message, font, sizeInterligne, borderColor, textColor,
        backgroundColor, text_justification, horizontal_position, vertical_position, pile);
    va_end(pile);
}
...
MLVAPI void MLV_draw_text_box_with_font_on_image_va(int x, int y, int width, int height, const char *message, MLV_Font *font,
    ...
    if (vasprintf(&complete_message, message, pile) == -1) { ERROR("Unexpected Error."); }
    internal_draw_text_box_with_font_on_image(x, y, width, height, complete_message, font, sizeInterligne, borderColor,
        textColor, backgroundColor, text_justification, horizontal_position, vertical_position, image->surface);
    free(complete_message);
}
...
MLVAPI void MLV_draw_text_box_with_font_on_image(int x, int y, int width, int height, const char *message, MLV_Font *font,
    ...
    va_list pile;
    va_start(pile, image);
    MLV_draw_text_box_with_font_on_image_va(x, y, width, height, message, font, sizeInterligne, borderColor, textColor,
        backgroundColor, text_justification, horizontal_position, vertical_position, image, pile);
    va_end(pile);
}
...
MLVAPI void MLV_draw_text_box_on_image_va(int x, int y, int width, int height, const char *message, int sizeInterligne,
    ...
    va_list pile) {
    MLV_draw_text_box_with_font_on_image_va(x, y, width, height, message, MLV_data->defaultFont, sizeInterligne,
        borderColor, textColor, backgroundColor, text_justification, horizontal_position, vertical_position, image,
        pile);
}
...
MLVAPI void MLV_draw_text_box_on_image(int x, int y, int width, int height, const char *message, int sizeInterligne,
    ...
    va_list pile;
    va_start(pile, image);
    MLV_draw_text_box_on_image_va(x, y, width, height, message, sizeInterligne, borderColor, textColor, backgroundColor,
        text_justification, horizontal_position, vertical_position, image, pile);
    va_end(pile);
}
