...
/**************************************************************************/
void initialize_default_font(const char *path_to_font, unsigned int size_font) {
    if (!MLV_data) {
...
void intialize_post_production_images() {
    ...
    MLV_data->post_production_image = MLV_create_image(MLV_data->width, MLV_data->height);
...
/**************************************************************************/
void initialise_graphic_window(unsigned int width, unsigned int height, int full_screen_is_required) {
...
#endif
    if (full_screen_is_required) { video_mode_flags |= SDL_FULLSCREEN; }
    MLV_data->full_screen_is_enable = full_screen_is_required;

    if (MLV_data->screen) { SDL_FreeSurface(MLV_data->screen); }
    ...
    if (MLV_data->save_screen) { SDL_FreeSurface(MLV_data->save_screen); }
    ...
    if (we_need_to_reinitialize_post_screen) { intialize_post_production_images(); }
}
...
void prepare_post_production_image() {
    MLV_draw_filled_rectangle_on_image(0, 0, MLV_get_window_width(), MLV_get_window_height(), MLV_data->ground_color,
        MLV_data->post_production_image);

    MLV_set_alpha_on_image(MLV_ALPHA_TRANSPARENT, MLV_data->post_production_image);
...
void MLV_update_window() {
    ...
    if (!MLV_data->post_producters) {
        ...
        boxColor(MLV_data->post_screen, 0, 0, MLV_data->width - 1, MLV_data->height - 1, MLV_data->ground_color);
        SDL_BlitSurface(MLV_data->screen, NULL, MLV_data->post_screen, &MLV_data->rectangle);
        SDL_BlitSurface(MLV_data->post_production_image->surface, NULL, MLV_data->post_screen, &MLV_data->rectangle);
...
void MLV_execute_at_exit(void (*function)(void *), void *data) {
    ...
    if (MLV_call_back_function_for_exit) { ERROR("The call back function for exiting program is yet initialized."); }
