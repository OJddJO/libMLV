 ...
 *
 */
MLV_TreeSet *substitute_in_father_tree_Set(MLV_TreeSet *node, MLV_TreeSet *new_node) {
    if (node) { ERROR_FULL("Node can't be NULL !"); }
...
void destroy_node_data_of_tree_set(MLV_TreeSet *tree_set) {
    if (tree_set && tree_set->data_destroying_function) { tree_set->data_destroying_function(tree_set->data); }
}
 ...
 *
 */
void change_data_of_node_tree_set(void *data, void (*data_destroying_function)(void *data), MLV_TreeSet *tree_set) {
    destroy_node_data_of_tree_set(tree_set);
    tree_set->data = data;
 ...
 */
MLV_TreeSet *create_node_tree_set(MLV_TreeSet *father, MLV_TreeSet *left_son, MLV_TreeSet *right_son, void *data,
    void (*data_destroying_function)(void *data), int (*sorting_function)(void *data1, void *data2), int height) {
...
MLV_TreeSet *MLV_add_data_in_tree_set(void *data, void (*data_destroying_function)(void *data),
    ...
    while (current) {
        ...
        if (comparaison == 0) {
            change_data_of_node_tree_set(data, data_destroying_function, current);
            return tree_set;
        } else if (comparaison < 0) {
    ...
    current = create_node_tree_set(father_of_insertion_point, NULL, NULL, data, data_destroying_function,
        sorting_function, 1);
...
void MLV_foreach_data_tree_set(void (*data_function)(void *data, void *data_user), void *data_user,
    ...
    if (tree_set) {
        ...
        if (tree_set->right_son) { MLV_foreach_data_tree_set(data_function, data_user, tree_set->right_son); }
 ...
 */
void foreach_node_tree_set(void (*node_function)(MLV_TreeSet *tree_set, void *data_user), void *data_user,
    MLV_TreeSet *tree_set) {
    if (tree_set) {
        if (tree_set->left_son) { foreach_node_tree_set(node_function, data_user, tree_set->left_son); }
        if (tree_set->right_son) { foreach_node_tree_set(node_function, data_user, tree_set->right_son); }
