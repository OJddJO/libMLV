...
void internal_scan_xml(const MLV_Xml *xml_data, const char *xpath, va_list pile, xmlXPathObjectPtr *result_search) {
    xmlXPathContextPtr context;

    char *complete_xpath;
    if (vasprintf(&complete_xpath, xpath, pile) < 0) { ERROR_FULL("Unexpected error"); }
...
char *internal_get_string_value_from_xml(const MLV_Xml *xml_data, int i, const char *xpath, va_list pile) {
    char *result = NULL;
    ...
    if (i < nodeset->nodeNr) {
        result = (char *)xmlNodeListGetString(xml_data->doc, nodeset->nodeTab[i]->xmlChildrenNode, 1);
    }
    xmlXPathFreeObject(result_search);
    return result;
}
