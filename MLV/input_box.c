...
inline
    ...
    MLV_get_size_of_text_with_font(input_box->informativeMessage, &width_message, &height_message, input_box->font);
    input_box->widthMessage = width_message;
    input_box->widthAnswer = input_box->width - input_box->widthMessage - 2 * input_box->bord[0];
    input_box->heightMessAns = (input_box->font->size > height_message) ? input_box->font->size : height_message;
...
void lock_input_box(MLV_Input_box *input_box) {
    if (SDL_SemWait(input_box->semaphore)) { ERROR_FULL("Probleme de semaphore"); }
}

void unlock_input_box(MLV_Input_box *input_box) {
    if (SDL_SemPost(input_box->semaphore)) { ERROR_FULL("Probleme de semaphore"); }
}
...
void lock_input_box_information() {
    if (SDL_SemWait(input_box_information.semaphore)) { ERROR_FULL("Probleme de semaphore"); }
}

void unlock_input_box_information() {
    if (SDL_SemPost(input_box_information.semaphore)) { ERROR_FULL("Probleme de semaphore"); }
}
...
inline
    ...
    int result = 0;
    if ((x >= input_box->top_left_corner_[0]) && (x <= input_box->bottom_right_corner_[0]) &&
        (y >= input_box->top_left_corner_[1]) && (y <= input_box->bottom_right_corner_[1]))
        result = 1;
    return result;
}
...
inline
    ...
    boxColor(input_box->apparence, 0, 0, input_box->width - 1, input_box->height - 1, input_box->backgroundColor);

    drawString(input_box->apparence, input_box->bord[0], input_box->bord[1], input_box->informativeMessage,
        input_box->textColor, input_box->font->font);

    if (input_box->widthAnswer > 0) {
        boxColor(input_box->answer, 0, 0, input_box->widthAnswer - 1, input_box->heightMessAns - 1,
        ...
        MLV_get_size_of_text_with_font(message_before_input_box, &width_message_before_input_box,
            &height_message_before_input_box, input_box->font);
        ...
        drawString(input_box->answer, decalage, 0, message, input_box->textColor, input_box->font->font);
        SDL_BlitSurface(input_box->answer, NULL, input_box->apparence, &(input_box->answerRectangle));
    }
    ...
    if ((!input_box->isListenning) || input_box->cursorIsVisible) {
        int abscisse;
        if (width_message_before_input_box > input_box->widthAnswer) {
            abscisse = input_box->bord[0] + input_box->widthMessage + input_box->widthAnswer - 2;
        } else {
            abscisse = input_box->bord[0] + input_box->widthMessage + width_message_before_input_box;
        }
        lineColor(input_box->apparence, abscisse, input_box->bord[1], abscisse,
            input_box->bord[1] + input_box->heightMessAns - 1, input_box->textColor);
    }

    rectangleColor(input_box->apparence, 0, 0, input_box->width - 1, input_box->height - 1, input_box->borderColor);
}
...
MLV_Input_box *create_input_box_with_font(int top_left_corner_X, int top_left_corner_Y, int width, int height,
...
MLV_Input_box *MLV_create_input_box_with_font_va(int top_left_corner_X, int top_left_corner_Y, int width, int height,
    ...
    MLV_Input_box *result = create_input_box_with_font(top_left_corner_X, top_left_corner_Y, width, height, borderColor,
...
MLV_Input_box *MLV_create_input_box_with_font(int top_left_corner_X, int top_left_corner_Y, int width, int height,
    ...
    va_list pile;
    va_start(pile, font);
    MLV_Input_box *result = MLV_create_input_box_with_font_va(top_left_corner_X, top_left_corner_Y, width, height,
        borderColor, textColor, backgroundColor, informativeMessage, font, pile);
    va_end(pile);
    return result;
}
...
MLV_Input_box *MLV_create_input_box(int top_left_corner_X, int top_left_corner_Y, int width, int height,
    ...
    MLV_Input_box *result = MLV_create_input_box_va(top_left_corner_X, top_left_corner_Y, width, height, borderColor,
        textColor, backgroundColor, informativeMessage, pile);
    va_end(pile);
    return result;
}
...
void change_informative_message_of_input_box(MLV_Input_box *input_box, const char *message) {
    lock_input_box(input_box);
...
int input_box_events_filter(const SDL_Event *event) {
    ...
    lock_input_box_information();
    switch (event->type) {
        case SDL_KEYUP: { // Une touche a ete appuyee
            if (input_box_information.input_box_activated) result = 0;
        } break;
        case SDL_KEYDOWN: { // Une touche a ete appuyee
            ...
            if (input_box_information.input_box_activated) {
                ...
                if (event->key.keysym.sym == SDLK_LEFT) {
                    ...
                    input_box_move_right_answer_NTS(input_box_information.input_box_activated);
                           ...
                           SDLK_UP) { // Si la touche est une fleche bas ou haut on se  deplace dans l'historique
                    ...
                    goDownInHistory_NTS(input_box_information.input_box_activated);
                           ...
                           SDLK_DELETE) { // Si la touche est une suppession on supprime une lettre
                    ...
                    char *character = MLV_convert_unicode_to_string(event->key.keysym.unicode);
                    int i = 0;
                    while (character[i] != '\0') {
                        add_carac_input_box_NTS(input_box_information.input_box_activated, character[i]);
                        i++;
                    }
        ...
        case SDL_MOUSEBUTTONDOWN: { // Si on est dans une zone de boîte de saisie on active
                                    //  l'enregistrement des données.
            MLV_Input_box *tmp = is_in_an_input_box_NTS(event->button.x, event->button.y);
            if (tmp) {
                lock_input_box(tmp);
