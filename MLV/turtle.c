...
MLVAPI void MLV_turtle_forward(MLV_Turtle *turtle, float distance) {
    ...
    if (turtle->write) {
        if (turtle->image) {
            MLV_draw_line_on_image(old_x, old_y, turtle->x, turtle->y, turtle->color, turtle->image);
        } else {
...
MLVAPI void MLV_turtle_write(MLV_Turtle *turtle, int write) {
    ...
    if (write) {
        if (turtle->image) {
            MLV_draw_point_on_image(turtle->x, turtle->y, turtle->color, turtle->image);
        } else {
...
MLVAPI void MLV_draw_turtle_on_image(MLV_Turtle *turtle, MLV_Image *image) {
    ...
    MLV_draw_filled_triangle_on_image(Ax, Ay, Bx, By, Cx, Cy, turtle->color, image);
}
...
void init_leonardo_turtle() {
    if (MLV_data->leonardo) { free_leonardo_turtle(); }
    ...
    MLV_turtle_go_to(MLV_data->leonardo->turtle, MLV_get_window_width() / 2, MLV_get_window_height() / 2);
