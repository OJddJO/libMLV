            ...
            switch (event.type) {
                case SDL_KEYDOWN: {
                    if (key_sym || key_mod || state) {
                        if (key_sym) { (*key_sym) = (MLV_Keyboard_button)event.key.keysym.sym; }
                        if (key_mod) { (*key_mod) = (MLV_Keyboard_modifier)event.key.keysym.mod; }
                        if (unicode) { (*unicode) = event.key.keysym.unicode; }
                        if (state) { (*state) = MLV_PRESSED; }
                        return MLV_KEY;
                    }
                }; break;
                case SDL_KEYUP: {
                ...
                case SDL_MOUSEMOTION: {
                    if (mouse_x || mouse_y) {
                        if (mouse_x) { (*mouse_x) = event.motion.x; }
                        if (mouse_y) { (*mouse_y) = event.motion.y; }
                        return MLV_MOUSE_MOTION;
                    }
                } break;
                case SDL_MOUSEBUTTONUP: {
                    if (mouse_x || mouse_y || mouse_button || state) {
                        if (mouse_x) { (*mouse_x) = event.button.x; }
                        if (mouse_y) { (*mouse_y) = event.button.y; }
                        if (mouse_button) { (*mouse_button) = (MLV_Mouse_button)event.button.button; }
                        if (state) { (*state) = MLV_RELEASED; }
                ...
                case SDL_USEREVENT: {
                    if (event.user.code == MLV_INPUT_BOX) {
                        if (texte || input_box) {
                            if (input_box) { (*input_box) = event.user.data1; }
                            if (texte) {
                                int size = strlen(event.user.data2) + 1;
                                (*texte) = MLV_MALLOC(size, char);
                                memcpy(*texte, event.user.data2, size);
                            }
...
MLVAPI MLV_Event MLV_wait_keyboard_or_mouse(MLV_Keyboard_button *sym, MLV_Keyboard_modifier *mod, int *unicode, int *mouse_x,
    ...
    while ((((resultat = MLV_wait_event(&tmp_sym, &tmp_mod, &tmp_unicode, NULL, NULL, &tmp_mouse_x, &tmp_mouse_y,
                  &mouse_button, &state)) != MLV_KEY) &&
               ((resultat != MLV_MOUSE_BUTTON) || (mouse_button != MLV_BUTTON_LEFT))) ||
           (state != MLV_PRESSED))
        ;
...
MLVAPI MLV_Event MLV_wait_keyboard_or_mouse_or_milliseconds(MLV_Keyboard_button *sym, MLV_Keyboard_modifier *mod, int *unicode,
    ...
    while (
        (event_is_not_valid =
                (((resultat = MLV_wait_event_or_milliseconds(&tmp_sym, &tmp_mod, &tmp_unicode, NULL, NULL, &tmp_mouse_x,
                       &tmp_mouse_y, &mouse_button, &state, milliseconds - (MLV_get_time() - time))) != MLV_KEY) &&
                    ((resultat != MLV_MOUSE_BUTTON) || (mouse_button != MLV_BUTTON_LEFT))) ||
                (state != MLV_PRESSED)) &&
        ((MLV_get_time() - time) < milliseconds))
        ;
...
MLVAPI MLV_Event MLV_convert_string_to_event(const char *event_string) {
    ...
    if (strcmp(event_string, "MLV_MOUSE_BUTTON") == 0) { return MLV_MOUSE_BUTTON; }
    if (strcmp(event_string, "MLV_MOUSE_MOTION") == 0) { return MLV_MOUSE_MOTION; }
