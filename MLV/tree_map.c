...
PairKeyData *create_pair_key_data(MLV_Key *key, void *data, void (*data_destroying_function)(void *data)) {
...
void free_pair_key_data(PairKeyData *pair) {
    if (pair) {
        ...
        if (pair->data_destroying_function) { pair->data_destroying_function(pair->data); }
    }
...
Wrapper_data_user *create_wrapper_data_user(void *data_user,
    void (*key_data_function)(MLV_Key *key, void *data, void *data_user)) {
...
void wrapper_key_data_function(void *data, void *data_user) {
    ...
    if (wrapper_data_user->key_data_function) {
        wrapper_data_user->key_data_function(pair->key, pair->data, wrapper_data_user->data_user);
    }
}
...
MLVAPI void MLV_foreach_key_data_tree_map(void (*key_data_function)(MLV_Key *key, void *data, void *data_user),
    ...
    MLV_foreach_data_tree_set(wrapper_key_data_function, wrapper_data, tree_map->set);
