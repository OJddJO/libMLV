...
SDL_Surface *create_surface(int width, int height) {
    ...
    return SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_SRCALPHA, width, height, MLV_BPP, rmask, gmask, bmask, amask);
#else
    return SDL_CreateRGBSurface(SDL_HWSURFACE | SDL_SRCALPHA, width, height, MLV_BPP, rmask, gmask, bmask, amask);
#endif
}
...
MLVAPI MLV_Image *MLV_load_image(const char *file_image) {
    ...
    image->surface = SDL_DisplayFormatAlpha(surface);
...
MLVAPI void MLV_vertical_image_mirror(MLV_Image *image) {
    ...
    for (i = 0; i < width / 2; i++) {
        for (j = 0; j < height; j++) {
            pixel_src = ((Uint32 *)image->surface->pixels) + j * image->surface->pitch / 4 + i;
            pixel_dst = ((Uint32 *)image->surface->pixels) + j * image->surface->pitch / 4 + width - 1 - i;
            tmp = *pixel_dst;
            *pixel_dst = *pixel_src;
            *pixel_src = tmp;
        }
...
MLVAPI void MLV_horizontal_image_mirror(MLV_Image *image) {
    ...
    for (i = 0; i < width; i++) {
        for (j = 0; j < height / 2; j++) {
            pixel_src = ((Uint32 *)image->surface->pixels) + j * image->surface->pitch / 4 + i;
            pixel_dst = ((Uint32 *)image->surface->pixels) + (height - 1 - j) * image->surface->pitch / 4 + i;
            tmp = *pixel_dst;
            *pixel_dst = *pixel_src;
            *pixel_src = tmp;
        }
...
MLVAPI void MLV_resize_image_with_proportions(MLV_Image *image, int width, int height) {
    ...
    if (width > 0) { scalar_x = ((double)width) / ((double)image->surface->w); }
    if (height > 0) { scalar_y = ((double)height) / ((double)image->surface->h); }
    if (scalar_x < 0) { scalar_x = scalar_y; }
    if (scalar_y < 0) { scalar_y = scalar_x; }
...
MLVAPI MLV_Image *MLV_copy_partial_image(const MLV_Image *image, int x, int y, int width, int height) {
    ...
    for (i = 0; i < width; i++) {
        for (j = 0; j < height; j++) {
            pixel_src = ((Uint32 *)image->surface->pixels) + (y + j) * image->surface->pitch / 4 + x + i;
            pixel_dst = ((Uint32 *)result->surface->pixels) + j * result->surface->pitch / 4 + i;
            *pixel_dst = *pixel_src;
        }
    }
...
MLVAPI void MLV_set_alpha_on_image(MLV_Alpha alpha, MLV_Image *image) {
    ...
    for (x = 0; x < width; x++) {
        for (y = 0; y < height; y++) {
            ...
            *pixel = (*pixel & (fmt->Rmask | fmt->Gmask | fmt->Bmask)) | alpha32;
        }
    }
...
void get_pixel_on_image(SDL_Surface *surface, int x, int y, int *red, int *green, int *blue, int *alpha) {
    SDL_LockSurface(surface);

    get_pixel_on_image_unsafe(surface, x, y, red, green, blue, alpha);
...
MLVAPI void MLV_draw_partial_image_on_image(const MLV_Image *source_image, int source_x, int source_y, int width, int height,
    ...
    SDL_BlitSurface(source_image->surface, &rectangle_source, destination_image->surface, &rectangle_destination);
}
