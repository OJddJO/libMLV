...
MLVAPI MLV_Animation *MLV_create_animation(uint32_t max_frames, uint32_t nb_layers, uint32_t nb_channels) {
    ...
    for (uint32_t i = 0; i < max_frames; i++) {
        if (nb_layers > 0) {
            ...
            for (uint32_t j = 0; j < nb_layers; j++) {
        ...
        if (nb_channels > 0) {
            ...
            for (uint32_t j = 0; j < nb_channels; j++) {
...
void user_is_modifying_animation(MLV_Animation *animation, uint32_t position) {
    if (position < animation->max_frames) {
        if (position >= animation->nb_frames) { animation->nb_frames = position + 1; }
    } else {
        char *text;
        int error = asprintf(&text,
            "Tou can-t add or modify an image at the position %d, "
            "you have created an animation with only %d images.",
            position, animation->max_frames);
        if (error == -1) { ERROR_FULL("Allocation memory problem."); }
        ERROR(text);
        MLV_FREE(text, char);
    }
}
...
MLVAPI void MLV_update_animation_player(MLV_Animation_player *animation_player) {
    ...
    frame = &animation_player->animation->frames[animation_player->current_frame];
...
MLVAPI void MLV_play_sounds_from_animation_player(MLV_Animation_player *animation_player, ...) {
    ...
    while (layer >= 0) {
        MLV_play_sound_from_animation_player(animation_player, layer);
        layer = va_arg(pile, int);
    }
...
MLVAPI MLV_Animation_book *MLV_load_animation_book(const char *xml_file, const char *image_directory,
    ...
    if (!MLV_path_exists(xml_file)) {
        fprintf(stderr, "The xml file %s doesn't exists.\n", xml_file);
        return NULL;
    }
    if (image_directory && (!strcmp(image_directory, "")) && MLV_path_exists(image_directory)) {
        fprintf(stderr, "The image directory %s doesn't exists.\n", image_directory);
        return NULL;
    }
    if (sound_directory && (!strcmp(sound_directory, "")) && MLV_path_exists(sound_directory)) {
        fprintf(stderr, "The sound directory %s doesn't exists.\n", sound_directory);
        return NULL;
    }
    ...
    result->nb_animations = MLV_get_number_of_objects_from_xml(xml, "/animation_book/animation");
    result->animations = MLV_MALLOC(result->nb_animations, MLV_Animation *);
    result->animation_names = MLV_MALLOC(result->nb_animations, char *);
    result->animation_name_to_id = MLV_create_tree_map();
    for (i = 0; i < result->nb_animations; i++) {
        if (MLV_get_string_value_from_xml(xml, &(result->animation_names[i]), "/animation_book/animation[%d]/name",
                i + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. The Field "
                "/animation_book/animation[%d]/name doesn't exist.\n",
                i + 1);
            exit(0);
        };
        insert_data_in_map(result->animation_name_to_id, result->animation_names[i], i);
    }

    result->nb_layers = MLV_get_number_of_objects_from_xml(xml, "/animation_book/layers/layer");
    result->layer_names = MLV_MALLOC(result->nb_layers, char *);
    result->layer_name_to_id = MLV_create_tree_map();
    for (i = 0; i < result->nb_layers; i++) {
        if (MLV_get_string_value_from_xml(xml, &(result->layer_names[i]), "/animation_book/layers/layer[%d]", i + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. The Field "
                "/animation_book/layers/layer[%d] doesn't exist.\n",
                i + 1);
            exit(0);
        }
        insert_data_in_map(result->layer_name_to_id, result->layer_names[i], i);
    }

    result->nb_channels = MLV_get_number_of_objects_from_xml(xml, "/animation_book/channels/channel");
    result->channel_names = MLV_MALLOC(result->nb_channels, char *);
    result->channel_name_to_id = MLV_create_tree_map();
    for (i = 0; i < result->nb_channels; i++) {
        if (MLV_get_string_value_from_xml(xml, &(result->channel_names[i]), "/animation_book/channels/channel[%d]",
                i + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. The Field "
                "/animation_book/channels/channel[%d] doesn't exist.\n",
                i + 1);
            exit(0);
        };
        insert_data_in_map(result->channel_name_to_id, result->channel_names[i], i);
    }

    result->nb_pages = MLV_get_number_of_objects_from_xml(xml, "/animation_book/page");
    result->page_names = MLV_MALLOC(result->nb_pages, char *);
    result->page_name_to_id = MLV_create_tree_map();
    for (i = 0; i < result->nb_pages; i++) {
        if (MLV_get_string_value_from_xml(xml, &(result->page_names[i]), "/animation_book/page[%d]/name", i + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. The Field "
                "/animation_book/page[%d]/name doesn't exist.\n",
                i + 1);
            exit(0);
        };
        insert_data_in_map(result->page_name_to_id, result->page_names[i], i);
    }
    result->pages = MLV_MALLOC(result->nb_pages, Animation_book_page);

    result->nb_sounds = MLV_get_number_of_objects_from_xml(xml, "/animation_book/sound");
    result->sound_names = MLV_MALLOC(result->nb_sounds, char *);
    result->sound_name_to_id = MLV_create_tree_map();
    for (i = 0; i < result->nb_sounds; i++) {
        if (MLV_get_string_value_from_xml(xml, &(result->sound_names[i]), "/animation_book/sound[%d]/name", i + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. The Field "
                "/animation_book/sound[%d]/name doesn't exist.\n",
                i + 1);
            exit(0);
        }
        insert_data_in_map(result->sound_name_to_id, result->sound_names[i], i);
    }
    result->sounds = MLV_MALLOC(result->nb_sounds, Animation_book_sound);
    ...
    for (id_page = 0; id_page < nb_pages; id_page++) {
        if (MLV_get_integer_value_from_xml(xml, &(result->pages[id_page].width), "/animation_book/page[%d]/width",
                id_page + 1) ||
            MLV_get_integer_value_from_xml(xml, &(result->pages[id_page].height), "/animation_book/page[%d]/height",
                id_page + 1) ||
            MLV_get_integer_value_from_xml(xml, &(result->pages[id_page].columns), "/animation_book/page[%d]/columns",
                id_page + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. One of those fields "
                "/animation_book/page[%d]/width or /animation_book/page[%d]/height "
                "or /animation_book/page[%d]/columns are missing or are not "
                "integers.\n",
                id_page + 1, id_page + 1, id_page + 1);
            exit(0);
        };
        ...
        int nb_images = MLV_get_number_of_objects_from_xml(xml, "/animation_book/page[%d]/image", id_page + 1);
        int image;
        for (image = 0; image < nb_images; image++) {
            char *layer;
            char *file;
            if (MLV_get_string_value_from_xml(xml, &(layer), "/animation_book/page[%d]/image[%d]/layer", id_page + 1,
                    image + 1) ||
                MLV_get_string_value_from_xml(xml, &(file), "/animation_book/page[%d]/image[%d]/file", id_page + 1,
                    image + 1)) {
                fprintf(stderr,
                    "The animation book xml is corrupted. One of those Fields : "
                    "/animation_book/page[%d]/image[%d]/layer or "
                    "/animation_book/page[%d]/image[%d]/file are missing.\n",
                    id_page + 1, image + 1, id_page + 1, image + 1);
                exit(0);
            }
            MLV_Key *key = MLV_string_to_key(layer);
            int id_layer = *((int *)MLV_get_data_from_tree_map(key, result->layer_name_to_id));
            MLV_free_key(key);

            char *full_path;
            if (MLV_path_is_absolute(file)) {
                full_path = MLV_build_path(file, NULL);
            } else {
                if (image_directory) {
                    full_path = MLV_build_path(image_directory, file, NULL);
                } else {
            ...
            if (!image) {
                fprintf(stderr, "The file %s is an unknow image or is corrupted.\n", full_path);
                exit(0);
            }
    ...
    for (id_sound = 0; id_sound < nb_sounds; id_sound++) {
        ...
        int nb_samples = MLV_get_number_of_objects_from_xml(xml, "/animation_book/sound[%d]/sample", id_sound + 1);
        int sample;
        for (sample = 0; sample < nb_samples; sample++) {
            char *channel;
            char *file;
            if (MLV_get_string_value_from_xml(xml, &(channel), "/animation_book/sound[%d]/sample[%d]/channel",
                    id_sound + 1, sample + 1) ||
                MLV_get_string_value_from_xml(xml, &(file), "/animation_book/sound[%d]/sample[%d]/file", id_sound + 1,
                    sample + 1)) {
                fprintf(stderr,
                    "The animation book xml is corrupted. One of those Fields : "
                    "/animation_book/sound[%d]/sample[%d]/channel or "
                    "/animation_book/sound[%d]/sample[%d]/file are missing.\n",
                    id_sound + 1, sample + 1, id_sound + 1, sample + 1);
                exit(0);
            }
            MLV_Key *key = MLV_string_to_key(channel);
            int id_channel = *((int *)MLV_get_data_from_tree_map(key, result->channel_name_to_id));
            MLV_free_key(key);
            ...
            if (MLV_path_is_absolute(file)) {
                ...
                if (sound_directory) {
                    full_path = MLV_build_path(sound_directory, file, NULL);
                } else {
                    full_path = MLV_build_path(dirname, file, NULL);
                }
            }
            ...
            if (!sound) {
                fprintf(stderr, "The sound file %s are unknwon or corrupted.\n", full_path);
                exit(0);
            }
    ...
    for (id_animation = 0; id_animation < nb_animations; id_animation++) {
        char *page_name;
        if (MLV_get_string_value_from_xml(xml, &(page_name), "/animation_book/animation[%d]/page", id_animation + 1)) {
            fprintf(stderr,
                "The animation book xml is corrupted. The field : "
                "/animation_book/animation[%d]/page is missing.\n",
                id_animation + 1);
            exit(0);
        }
        MLV_Key *key = MLV_string_to_key(page_name);
        int id_page = *((int *)MLV_get_data_from_tree_map(key, result->page_name_to_id));
        MLV_free_key(key);
        int width = result->pages[id_page].width;
        int height = result->pages[id_page].height;
        int columns = result->pages[id_page].columns;
        int nb_frames =
            MLV_get_number_of_objects_from_xml(xml, "/animation_book/animation[%d]/frame", id_animation + 1);
        result->animations[id_animation] = MLV_create_animation(nb_frames, nb_layers, nb_channels);
        int id_frame;
        for (id_frame = 0; id_frame < nb_frames; id_frame++) {
            MLV_Sound **sounds = NULL;
            MLV_Image **images = NULL;
            char *sound;
            MLV_get_string_value_from_xml(xml, &(sound), "/animation_book/animation[%d]/frame[%d]/sound",
                id_animation + 1, id_frame + 1);
            if (sound) {
                MLV_Key *key = MLV_string_to_key(sound);
                int id_sound = *((int *)MLV_get_data_from_tree_map(key, result->sound_name_to_id));
                MLV_free_key(key);
                sounds = result->sounds[id_sound].channels;
            }
            int panel;
            if (MLV_get_integer_value_from_xml(xml, &(panel), "/animation_book/animation[%d]/frame[%d]/panel",
                    id_animation + 1, id_frame + 1)) {
                fprintf(stderr,
                    "The animation book xml is corrupted. The field : "
                    "/animation_book/animation[%d]/frame[%d]/panel is missing or "
                    "is not an integer.\n",
                    id_animation + 1, id_frame + 1);
                exit(0);
            }
            ...
            for (id_layer = 0; id_layer < nb_layers; id_layer++) {
                ...
                if (result->pages[id_page].layers[id_layer]) {
                    images[id_layer] =
                        MLV_copy_partial_image(result->pages[id_page].layers[id_layer], x, y, width, height);
                } else {
                    images[id_layer] = NULL;
                }
            }
            int time;
            if (MLV_get_integer_value_from_xml(xml, &(time), "/animation_book/animation[%d]/frame[%d]/time",
                    id_animation + 1, id_frame + 1)) {
                fprintf(stderr,
                    "The animation book xml is corrupted. The field : "
                    "/animation_book/animation[%d]/frame[%d]/time is missing or is "
                    "not an integer.\n",
                    id_animation + 1, id_frame + 1);
                exit(0);
            };
            MLV_add_frame_in_animation(images, sounds, time, result->animations[id_animation]);
...
MLVAPI void MLV_free_animation_book(MLV_Animation_book *animation_book) {
    ...
    for (id_sound = 0; id_sound < animation_book->nb_sounds; id_sound++) {
        int id_channel;
        for (id_channel = 0; id_channel < animation_book->nb_channels; id_channel++) {
            if (animation_book->sounds[id_sound].channels[id_channel]) {
                MLV_free_sound(animation_book->sounds[id_sound].channels[id_channel]);
            }
        }
    ...
    for (id_page = 0; id_page < animation_book->nb_pages; id_page++) {
        ...
        for (id_layer = 0; id_layer < animation_book->nb_layers; id_layer++) {
            if (animation_book->pages[id_page].layers[id_layer]) {
                MLV_free_image(animation_book->pages[id_page].layers[id_layer]);
            }
        }
